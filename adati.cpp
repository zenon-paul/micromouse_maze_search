...
void Adati::from_index_to_coordinate(int i, int* x, int* y) {
	*x = i % WID;
	*y = (i - *x) / WID;
}
void Adati::from_index_to_index(int index_from, int dx, int dy, int* index_to) {
	int x = 0;
	int y = 0;
...
void Adati::from_coordinate_to_index(int x, int y, int* i) {
	*i = WID * y + x;
}

int Adati::shortcut_judge(int index) {
	if (index == goal_index) {
		return 1;
	}
	int count = 0;
...
void Adati::set_adati(int sx, int sy, int gx, int gy) {
...
void Adati::adati() {
	...
	for (int i = 0; i < 4;i++) {
		...
		child_cost = diji.get_cost(child_index);
...
void Adati::step_back(int index1, int index2) {

}
vector<int> Adati::step_back2(int index1, int index2) {
...
vector<int> Adati::get_adati_path() {//この方法だとうまくいかない
